...
namespace reflectionCS2.game.Structs
{
    internal struct Matrix
    {
        ...
        {
            var wInv = 1.0 / (M14 * (double)value.X + M24 * (double)value.Y + M34 * (double)value.Z + M44);
            return new Vector3
            (
                (float)((M11 * (double)value.X + M21 * (double)value.Y + M31 * (double)value.Z + M41) * wInv),
                (float)((M12 * (double)value.X + M22 * (double)value.Y + M32 * (double)value.Z + M42) * wInv),
                (float)((M13 * (double)value.X + M23 * (double)value.Y + M33 * (double)value.Z + M43) * wInv)
            );
        }
