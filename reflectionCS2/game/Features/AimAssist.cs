...
namespace reflectionCS2.game.Features
{
    public static class AimAssist
    {
        ...
        public static void Draw(Graphics g)
        {
            ...
            Brushes.FOVColor.Color = _targetPtr == nint.Zero ? new Color(255, 0, 0, 100) : new Color(0, 255, 0, 100);
            float stroke = _targetPtr == nint.Zero ? 2 : 1;
        ...
        private static void Aim(Entity entity)
        {
            ...
            if (TryMouseMove(aimPixels))
                Thread.Sleep(Configuration.Current.AimDelay - 5);
        }
        ...
        private static void UpdateAim()
        {
            ...
            if (vkBindXBtn.state == Input.KeyState.DOWN && _targetPtr == nint.Zero)
                FindTarget();
            else if (vkBindXBtn.state == Input.KeyState.NONE)
            {
                _targetPos = Vector3.Zero;
                _targetPtr = nint.Zero;
        ...
        private static float FindBone(Entity entity, float minDistance, bool displayPoint = false)
        {
            ...
            {
                Enum.TryParse(bone.bone.ToString(), out HitboxBone hbb);
        ...
        {
            nint AddressBase = Memory.Read<nint>(Memory.ClientPtr + ClientOffsets.dwLocalPlayerPawn);
            ...
            nint pCameraServices = Memory.Read<nint>(AddressBase + C_BasePlayerPawn.m_pCameraServices);
        ...
        internal static nint _targetPtr = nint.Zero;
